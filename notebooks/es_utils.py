...
class MLPMultilabel:
    ...
    def avg_multilabel_BA_2(self, y_truei, y_predi):
    ...
    def test_BA(self, model, x_test_norm, y_test):
        ...
        return self.avg_multilabel_BA_2(y_test, y_pred)

    def build_base_model(self, input_dim, num_classes, neurons_1=16, neurons_2=None, l2_val=0.01) -> Sequential:
        model = Sequential()
    ...
    def hypertunning(self, x_train_norm, y_train, ):
        ...
        tuner = kt.RandomSearch(partial_hyper,
                                objective=kt.Objective('val_avg_multilabel_BA_2', direction="max"),
                                # 'val_avg_multilabel_BA_2',
                                max_trials=100,
...
class HAR:
    ...
    def __init__(self, config: dict) -> None:
        ...
        if self.config['df'] is None:
            self.data = DataProcessingExtrasensory(self.load(self.config['df_path']),
                                                   labels=self.config['labels'])  # TODO: attention
        else:
...
def remove_nans_np(x, y):
    ...
    return np.ma.compressed(marra), np.ma.compressed(
        marrb)  # np.ma.compressed(marra).reshape(-1, x.shape[1]), np.ma.compressed(marrb).reshape(-1, x.shape[1])
